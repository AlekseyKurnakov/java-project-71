### Hexlet tests and linter status:
[![Actions Status](https://github.com/AlekseyKurnakov/java-project-71/actions/workflows/hexlet-check.yml/badge.svg)](https://github.com/AlekseyKurnakov/java-project-71/actions)
[![Checkstyle](https://github.com/<USER>/<REPO>/actions/workflows/checkstyle.yml/badge.svg?branch=main)](https://github.com/<USER>/<REPO>/actions/workflows/checkstyle.yml)
---

## Демонстрация (asciinema)
- [Differ](https://asciinema.org/a/tnhtFH6priHxM6Z6)